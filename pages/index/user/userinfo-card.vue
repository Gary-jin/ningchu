<template>
	<view>
		<view class="sh-userinfo-box">
			<view class="head-wrap" :style="{ background: detail.image ? `url(${detail.image}) no-repeat center / 100% 100%` : detail.color }">
				...
				>
					...
					<view slot="right" class="sing" @tap="scanCode">
						<image src="../../../static/images/sing.png" mode="widthFix"></image>
					</view>
				</shopro-navbar>
				...
				<view class="user-head u-flex u-row-between">
					<view class="u-flex">
						...
						<view class="info-box">
							<view class="u-flex" >
								<view class="head-img-wrap" @tap="$Router.push('/pages/user/info')">
								...
								<view class="">
									<text class="user-name u-ellipsis-1">{{ userInfo.nickname || '请登录~' }}</text></br>
									<view class="inviteBox">
										<text>邀请码:{{userInfo.invite_code}}</text>
										<button class="u-reset-button bindPhone1" @click="copyInvite(userInfo.invite_code)">复制</button>
									</view>
								</view>
							</view>
						</view>
						...
						<!-- 等级 -->
						<!-- <view v-if="userInfo.is_leader == 2" class="grade-tag tag-box u-flex">
							<text class="tag-title">董事</text>
						</view> -->
					</view>
					<view class="u-flex" v-if="userInfo.nickname">
						...
						<!-- 只展示二维码 -->
						<button class=" u-reset-button code-btn" @tap="onShareCode"><text class="iconfont icon-qrcode"></text></button>
					</view>
				</view>
			</view>
		</view>
		...
		<!-- 二维码 -->
		<view class="cu-modal" :class="{ show: showCode }" @tap="showCode = false">
			<view class="cu-dialog" style="width: 600rpx;">
				<view class="modal-box">
					<view class="modal-head">邀请二维码</view>
					<view class="modal-content qRImg">
						<canvas id="qrcode" canvas-id="qrcode" style="width: 400rpx;height: 400rpx;" />
					</view>
					<view class="modal-bottom u-flex u-col-center">
						<button class="u-reset-button modal-btn cancel-btn" :hover-stay-time="100" hover-class="btn-hover" @tap="showCode = false">取消</button>
						<button class="u-reset-button  modal-btn save-btn" :hover-stay-time="100" hover-class="btn-hover" @tap="download">下载</button>
					</view>
				</view>
			</view>
		</view>
...
import uQRCode from '@/shopro/poster/QS-SharePoster/uqrcode.js'
export default {
	components: {},
	data() {
		return {
	...
	computed: {
		...mapGetters(['isLogin', 'userInfo', 'userOtherData','shareInfo']),
		showRefresh() {
			if (this.isLogin) {
	...
	methods: {
		...
		sign(id) {
			let that = this;
			that.$http('api.clogSign', {id}).then(res => {
				uni.showModal({
				    title: '提示',
				    content: res.msg,
					showCancel:false,
