		...
		<view v-else-if="isConnected && isRefresh" class="content-box">
			...
			<view class="template-box">
				<block v-for="(item, index) in homeTemplate" :key="item.id">
					...
					<!-- 滑动宫格 -->
					<sh-grid-swiper v-if="item.type === 'menu'&& userInfo.id!='302'" :list="item.content.list" :oneRowNum="item.content.style"></sh-grid-swiper>
					...
					<!-- 优惠券 -->
					<sh-coupon v-if="item.type === 'coupons'&& userInfo.id!='302'" :detail="item.content"></sh-coupon>
					<!-- 秒杀-->
					<sh-seckill v-if="item.type === 'seckill'&& userInfo.id!='302'" :detail="item.content"></sh-seckill>
					<!-- 拼团 -->
					<sh-groupon v-if="item.type === 'groupon'&& userInfo.id!='302'" :detail="item.content"></sh-groupon>
					<!-- 富文本 -->
					<sh-richtext v-if="item.type === 'rich-text'" :richId="item.content.id"></sh-richtext>
					<!-- 功能标题 -->
					<sh-title-card v-if="item.type === 'title-block'&& userInfo.id!='302'" :title="item.content.name" :bgImage="item.content.image" :titleColor="item.content.color"></sh-title-card>
					<!-- 直播 -->
					<!-- #ifdef MP-WEIXIN -->
			...
			<shopro-modal></shopro-modal>
			<shopro-bounty></shopro-bounty>
			<!-- #endif -->
			<!-- #ifdef H5 -->
