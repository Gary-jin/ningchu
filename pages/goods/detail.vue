		...
		<view class="detail_box" v-else>
			...
			<view class="tabbar-foot safe-area-inset-bottom" v-if="goodsInfo.id">
				<view class="detail-foot_box safe-area-inset-bottom  u-flex">
					<!-- foot左侧 -->
					<view class="left u-flex" v-if="goodstype !=='agency'">
						<!-- 积分foot -->
					...
					<!-- foot右侧 -->
					<view class="detail-right" v-if="goodstype !=='agency'">
						<!-- 积分按钮 -->
					...
					<view class="detail-right" v-if="goodstype =='agency'">
						<view class="detail-btn-box u-flex u-row-around" >
							<button class=" u-reset-button tool-btn pay-btn" style="width: 80%;" @tap="goPay">立即购买</button>
						</view>
					</view>
...
export default {
	...
	async onLoad() {
		...
		this.goodstype = this.$Route.query.goodstype?this.$Route.query.goodstype:''
		switch (type) {
			case 'score':
