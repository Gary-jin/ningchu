...
<template>
	<view class="pay-method-wrap">
		<view class="u-flex-col u-col-center money-box" v-if="orderDetail.total_fee">
			...
			<view class="money">{{score?orderDetail.total_fee: (Number(orderDetail.total_fee) + Number(orderDetail.repurchase_score_fee)).toFixed(2)}}</view>
		...
		<u-radio-group v-if="initPayment.length" class="pay-box" v-model="payType" active-color="#f0c785">
			...
			<!-- 复购积分 -->
			<view class="u-flex u-row-between pay-item" v-if="orderDetail.repurchase_score_fee>0" >
				<view class="">是否使用复购积分({{userInfo.repurchase_score}})</view>
				<u-switch v-model="score" @change="change"></u-switch>
			</view>
			
		</u-radio-group>

		<button class="u-reset-button pay-btn" @tap="confirmPay">
			<!-- 确认支付 ￥{{ orderDetail.total_fee || '0.00' }} {{score?"  复购积分:"+orderDetail.repurchase_score_fee:""}} -->
			{{score?'确认支付 ￥'+(orderDetail.total_fee || '0.00')+'  复购积分:'+orderDetail.repurchase_score_fee :
			'确认支付 ￥'+(Number(orderDetail.total_fee) + Number(orderDetail.repurchase_score_fee)).toFixed(2)
			}}
		</button>
	</view>
</template>
...
export default {
	...
	computed: {
		...mapGetters(['initPayment','userInfo'])
	},
	onShow() {
	...
	onLoad() {
		...
		this.gstype = this.$Route.query.gstype?this.$Route.query.gstype:''
		
		// console.log('this.gstype',this.gstype);
	},
	onHide() {
		clearInterval(timer);
		timer = null;
		...
		confirmPay() {
			...
			if (that.payType === 'wallet') {
				uni.showModal({
					title: '支付提示',
					confirmColor: '#f0c785',
					content: `是否确认使用余额支付:${that.score?(that.orderDetail.total_fee || '0.00') :(Number(that.orderDetail.total_fee) + Number(that.orderDetail.repurchase_score_fee)).toFixed(2) || '0.00'}元?`,
					success: res => {
						if (res.confirm) {
							pay = new Pay(that.payType, that.orderDetail,that.gstype,that.score);
						}
					}
				});
			} else {
		...
		getOrderDetail() {
			...
			that.$http('order.detail', {
				...
				if (res.code === 1) {
					...
					if(res.data.repurchase_score_fee>0){
						that.getUserInfo()
						that.score = true
					} else{
						that.score = false
