...
export default {
	...
	setShareInfo(scene = {
		...
		shareInfo.path = `${store.getters.initShop?.invite_url}?code=${store.state.user.userInfo.id}`;
		// #endif
		// shareInfo.copyLink = `${store.getters.initShop?.domain}?${query}`;
		shareInfo.copyLink = `${store.getters.initShop?.invite_url}?code=${store.state.user.userInfo.id}`;
