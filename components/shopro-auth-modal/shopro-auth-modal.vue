	...
	<view class="cu-modal bottom-modal" :class="{ show: showAuth }" @tap="closeAuthModal" style="z-index: 10080;">
		<view class="login-wrap cu-dialog form-wrap  safe-area-inset-bottom" @tap.stop style="border-radius: 20rpx 20rpx 0 0;">
			...
			<view v-if="authType === 'register'">
				...
				<view v-if="initShop.invite_code == 1" class="form-item u-border-bottom">
					<view class="item-content u-flex u-col-center">
						<view class="item-title">邀请码</view>
						<u-input
							class="u-m-r-10 u-flex-1"
							placeholder="请输入邀请码"
							@blur="checkValue2(form['register'].data.invite_code)"
							@input="checkValue2(form['register'].data.invite_code)"
							:placeholderStyle="placeholderStyle"
							v-model="form['register'].data.invite_code"
							type="text"
						></u-input>
					</view>
					<view class="message-error">{{ form['register'].error.invite_code || '' }}</view>
				</view>
				<view class="form-item u-border-bottom">
					<view class="item-content u-flex u-col-center">
						<view class="item-title">验证码</view>
			...
			<!-- 第三方登录 -->
			<view v-if="initShop.invite_code != 1">
...
export default {
			...
			form: {
				...
				register: {
					...
					rules: {
						...
						mobile: FormValidate.mobile,
						password: FormValidate.password,
						invite_code: FormValidate.invite_code
					},
					error: {
						mobile: '', // 手机号
						code: '', // 验证码
	...
	onLoad() {
		console.log('this.FormValidate.invite_code',this.FormValidate.invite_code);
	},
	mounted() {
	},
	methods: {
		...
		checkValue1(){
			...
			if(this.initShop.invite_code == 1){
				if(!this.form['register'].data.invite_code){
					this.form['register'].error.invite_code = '请输入邀请码'
					return false;
					
				} else {
					this.form['register'].error.invite_code = ''
					return true;
				}
			}
		},
		...
		async registerSubmit() {
			let that = this;
			let invite = {};
			if(that.initShop.invite_code == 1){ //必填
				that.checkValue1()
				invite={
					invite_code: that.form['register'].data.invite_code
				}
			}
			(await that.validateSubmit())  &&
				that.$http(
